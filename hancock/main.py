...
def main():
    ...
    parser.add_argument('--output', type=str, default='blobs/linux_functions.json', help='Path to the output file')
    ...
    output = subprocess.run(['git', 'checkout', '-f', tags[0][1]], cwd=args.repo)

    print(output)

    # Get the functions all the files with the first tag 
    files = get_files_for_tag(tags[0][1], args.repo)
    print(f"Found {len(files)} files in {tags[0][1]}")

    # parallel extraction of functions from all files
    functions = process_files(files, args.repo)
    print(f"Found {len(functions)} functions in {len(files)} files")
    # saved the functions to a file
